...
document.addEventListener("DOMContentLoaded", function () {
  ...
  saveDefaultsBtn.addEventListener("click", () => {
    const paths = defaultExpansion.value
      .split(",")
      .map((p) => p.trim())
      .filter((p) => p);
  ...
  function generateExpansionPaths(schema, maxDepth = 3) {
    ...
    function findNestedRelations(collectionName, currentPath = [], depth = 0) {
      ...
      collection.fields.forEach((field) => {
        if (field.type === "relation") {
          const relatedCollection = schema.find(
          ...
          if (!currentPath.includes(relatedCollection.name)) {
            ...
            const nestedPaths = findNestedRelations(
  ...
  function populateCollections(schema) {
    ...
    schema.forEach((collection) => {
      ...
      collectionEl.innerHTML = `
                <div class="collection-check">
                    <input type="checkbox" id="col-${name}" class="collection-select" ${
        isChecked ? "checked" : ""
      }>
                </div>
    ...
    document.querySelectorAll(".collection-select").forEach((checkbox) => {
      checkbox.addEventListener("change", function () {
        ...
        localStorage.setItem(
          "collectionSelection",
          JSON.stringify(collectionSelectionState)
        );
      });
    });
    ...
    document.querySelectorAll(".save-title").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        localStorage.setItem(
          "collectionTitles",
          JSON.stringify(collectionTitles)
        );
  ...
  function getCollectionTitle(collectionName) {
    return (
      collectionTitles[collectionName] ||
      `${
        collectionName.charAt(0).toUpperCase() + collectionName.slice(1)
      } Collection`
    );
  ...
  function mapFieldToSchema(field, expansionSettings) {
    ...
    if (ftype === "bool") {
      ...
      if (maxSelect > 1) {
        ...
        schema.items = { type: "string" };
        description +=
          ". When expanded, this becomes an array of the related records.";
      } else {
        schema.type = "string";
        description +=
          ". When expanded, this becomes the related record object.";
      }
  ...
  // Function to build all authentication endpoints
  function buildAuthEndpoints(collection, customTitle, schemaName) {
        const name = collection.name;

    // Detect required fields for registration
    const registerProperties = {};
    const registerRequired = ["password", "passwordConfirm"];

    collection.fields.forEach((field) => {
      // Skip system fields and relations
      if (field.name.startsWith("@") || field.type === "relation") return;

      // Add field to properties
      registerProperties[field.name] = mapFieldToSchema(field);

      // Mark as required if needed
      if (field.required && field.name !== "password") {
    ...
    return {
        ...
        [`/collections/${name}/records`]: {
            post: {
                summary: `Register new ${customTitle}`,
                tags: [customTitle],
                requestBody: {
                    content: {
                        "application/json": {
      ...
      [`/collections/${name}/auth-with-password`]: {
        post: {
          summary: `Authenticate ${customTitle} with password`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/request-verification`]: {
        post: {
          summary: `Request ${customTitle} verification`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/confirm-verification`]: {
        post: {
          summary: `Confirm ${customTitle} verification`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/request-password-reset`]: {
        post: {
          summary: `Request ${customTitle} password reset`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/confirm-password-reset`]: {
        post: {
          summary: `Confirm ${customTitle} password reset`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/request-email-change`]: {
        post: {
          summary: `Request ${customTitle} email change`,
          tags: [customTitle],
          security: [{ bearerAuth: [] }], // Added security requirement
          requestBody: {
            content: {
              "application/json": {
                schema: {
      ...
      [`/collections/${name}/confirm-email-change`]: {
        post: {
          summary: `Confirm ${customTitle} email change`,
          tags: [customTitle],
          requestBody: {
            content: {
              "application/json": {
      ...
      [`/collections/${name}/auth-refresh`]: {
        post: {
          summary: `Refresh ${customTitle} authentication token`,
          tags: [customTitle],
          security: [{ bearerAuth: [] }], // Added security requirement
          responses: {
            200: {
           ...
           // Auth with OAuth2
           [`/collections/${name}/auth-with-oauth2`]: {
            post: {
              summary: `Authenticate ${customTitle} with OAuth2`,
              tags: [customTitle],
              requestBody: {
                content: {
                  "application/json": {
    ...
    //   // List auth methods
    //   [`/collections/${name}/list-auth-methods`]: {
    //     get: {
    //       summary: `List ${customTitle} authentication methods`,
    //       tags: [customTitle],
    //       responses: {
    //         200: {
    ...
    //   // List external auths
    //   [`/collections/${name}/list-external-auths`]: {
    //     get: {
    //       summary: `List ${customTitle} external authentications`,
    //       tags: [customTitle],
    //       responses: {
    //         200: {
    //           description: "List of external authentications",
    //           content: {
    //             "application/json": {
    //               schema: {
    //                 type: "array",
    ...
    //   [`/collections/${name}/unlink-external-auth`]: {
    //     delete: {
    //       summary: `Unlink ${customTitle} external authentication`,
    //       tags: [customTitle],
    //       requestBody: {
    //         content: {
    //           "application/json": {
    ...
    //       responses: {
    //         204: {
    //           description: "External authentication unlinked",
    //         },
    //       },
    //     },
    //   },
  ...
  function buildOpenapiSpec(schema, expansionSettings, expansionPaths) {
    ...
    schema.forEach((collection) => {
      ...
      if (ctype === "auth") {
        const authEndpoints = buildAuthEndpoints(collection, customTitle, schemaName);
        // const authEndpoints = buildAuthEndpoints(name, customTitle, schemaName);
        Object.assign(openapi.paths, authEndpoints);
      }
    });
  ...
  // Schema normalization for different PocketBase versions
